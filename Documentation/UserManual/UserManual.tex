...
\newcommand{\myshorttitle}{X-RAAS 2.7}
\newcommand{\builddate}{\today}

\title{\myshorttitle User Manual}
...
\fancyfoot[L]{\builddate}
\fancyfoot[C]{\myshorttitle}
...
X-RAAS supports three kinds of saved configurations, depending on the configuration file name
found in the X-plane preferences folder. The files are loaded in the following
order (the 1st file founded is used):

\begin{description}

\item[Airline configuration:] This configuration is stored in the global
X-Plane configuration folder in the file X-RAAS\_AIRCRAFT-FOLDERNAME\_LIVERY-FOLDERNAME.cfg\\
(i.e.\ ``\texttt{Output/preferences/X-RAAS\_ToLiss\_A32N\_AIR\_France\_F-GNEO.cfg}'').\\\

\item[Aircraft configuration:] This configuration is stored in the global
X-Plane configuration folder in the file X-RAAS\_AIRCRAFT-FOLDERNAME.cfg\\
(i.e.\ ``\texttt{Output/preferences/X-RAAS\_ToLiss\_A32N.cfg}'').\\\

\item[Global configuration:] This configuration is stored in the global
X-Plane configuration folder
(i.e.\ ``\texttt{Output/preferences/X-RAAS.cfg}'').\\\
...
\hspace*{4em}\fileicon{file.png} X-RAAS.cfg\hspace*{4em}{\small
\textsf{$\leftarrow$ overrides settings shared by all aircraft}}\\
\hspace*{4em}\fileicon{file.png} X-RAAS\_AIRCRAFT-FOLDERNAME.cfg\hspace*{3em}\\{\small
\hspace*{7em}\textsf{$\uparrow$ overrides global settings only for this AIRCRAFT}}\\
\hspace*{4em}\fileicon{file.png} X-RAAS\_AIRCRAFT-FOLDERNAME\_LIVERY-FOLDERNAME.cfg\hspace*{3em}\\{\small
\hspace*{7em}\textsf{$\uparrow$ overrides aircraft settings only for this LIVERY}}
}
