...
enum { CONFIG_GUI_CMD, DBG_GUI_TOGGLE_CMD, RAAS_RESET_CMD, RECREATE_CACHE_CMD };
...
static void save_config(conf_target_t);
...
static void nd_alert2str(int level, char buf[32]) {
  switch (level) {
...
#define GEN_BOOL_CONF(widget)                                                  \
  append_format(                                                               \
      ...
      XPGetWidgetProperty(buttons.widget, xpProperty_ButtonState, NULL)        \
          ? "true"                                                             \
          : "false")
  ...
  for (int i = 0; i < NUM_MONITORS; i++)
    append_format(
        ...
        XPGetWidgetProperty(buttons.monitors[i], xpProperty_ButtonState, NULL)
            ? "true"
            : "false");
...
#define GEN_TEXT_CONF(text_field)                                              \
  do {                                                                         \
    char buf[32];                                                              \
    XPGetWidgetDescriptor(text_fields.text_field, buf, sizeof(buf) - 1);       \
    if (strlen(buf) != 0 && strcmp(buf, "default") != 0)                       \
      append_format(&conf_text, &conf_sz, "%s = %s" NEWLINE, #text_field,      \
                    buf);                                                      \
  } while (0)
...
#define GEN_FRACT_CONF(scrollbar, multiplier)                                  \
  do {                                                                         \
    double value =                                                             \
        XPGetWidgetProperty(scrollbars.scrollbar,                              \
                            xpProperty_ScrollBarSliderPosition, NULL) *        \
        multiplier;                                                            \
    append_format(&conf_text, &conf_sz, "%s = %g" NEWLINE, #scrollbar, value); \
  } while (0)
...
void SettingsWindow::LoadConfig(void) { config = xraas_state->config; }
...
#define SLIDER_FLOAT(var, label, tool_tip, format, min, max, multiplier)       \
  do {                                                                         \
...
#define INPUT_DOUBLE(var, label, tool_tip, suffix, step, step_fast, min, max, format)                 \
  ...
  if (ImGui::BeginTable("##main_table", 2, ImGuiTableFlags_SizingStretchSame,
    ...
    BLANK_ROW;
    SECTION_TILE("Customize parameters");
  ...
  bool save_button = CenterButton("Save preferences");
  if (save_button) {
    // SetVisible(B_FALSE);
...
static void save_config(conf_target_t target) {
  char *config;
  ...
  case CONFIG_TARGET_AIRCRAFT:
    ...
    break;
  case CONFIG_TARGET_GLOBAL:
    XPSetWidgetDescriptor(text_fields.status_msg, "Saved global configuration");
    break;
  }
}
...
static void menu_cb(void *menu, void *item) {
  ...
  switch (cmd) {
  case CONFIG_GUI_CMD:
    XPShowWidget(main_win);                            /// TODO remove it
    XPSetWidgetDescriptor(text_fields.status_msg, ""); /// TODO remove it
...
static XPWidgetID layout_text_field(XPWidgetID window, tooltip_set_t *tts,
                                    int x, int y, const char *label,
                                    int max_chars, const char *units,
                                    const char *tooltip, bool_t wide) {
  XPWidgetID widget;
  ...
  if (units != NULL)
    (void)create_widget_rel(x + TEXT_FIELD_WIDTH * 0.95, y, B_FALSE,
                            TEXT_FIELD_WIDTH * 0.05, TEXT_FIELD_HEIGHT - 5, 1,
                            units, 0, window, xpWidgetClass_Caption);
  widget = create_widget_rel(x + TEXT_FIELD_WIDTH * cap_width, y + 2, B_FALSE,
                             TEXT_FIELD_WIDTH * (0.95 - cap_width),
                             TEXT_FIELD_HEIGHT - 5, 1, "", 0, window,
                             xpWidgetClass_TextField);
  XPSetWidgetProperty(widget, xpProperty_TextFieldType, xpTextEntryField);
  XPSetWidgetProperty(widget, xpProperty_Enabled, 1);
  if (max_chars != 0)
    XPSetWidgetProperty(widget, xpProperty_MaxCharacters, max_chars);
  if (tooltip != NULL)
...
static void create_main_window(void) {
  ...
  main_win = create_widget_rel(
      100, 100, B_FALSE, MAIN_WINDOW_WIDTH, MAIN_WINDOW_HEIGHT, 0,
      XRAAS_MENU_NAME " Configuration", 1, NULL, xpWidgetClass_MainWindow);
  ...
  (void)create_widget_rel(LAYOUT_START_X + WINDOW_MARGIN, LAYOUT_START_Y - 15,
                          B_FALSE, COLUMN_X, 12, 1, "Global settings", 0,
                          main_win, xpWidgetClass_Caption);
  (void)create_widget_rel(LAYOUT_START_X, LAYOUT_START_Y, B_FALSE, COLUMN_X,
                          COLUMN_Y, 1, "", 0, main_win,
                          xpWidgetClass_SubWindow);

  (void)create_widget_rel(
      LAYOUT_START_X + WINDOW_MARGIN + COLUMN_X + WINDOW_MARGIN,
      LAYOUT_START_Y - 15, B_FALSE, COLUMN_X, 12, 1, "Monitor settings", 0,
      main_win, xpWidgetClass_Caption);
  (void)create_widget_rel(LAYOUT_START_X + COLUMN_X + WINDOW_MARGIN,
                          LAYOUT_START_Y, B_FALSE, COLUMN_X, COLUMN_Y, 1, "", 0,
                          main_win, xpWidgetClass_SubWindow);

  (void)create_widget_rel(
      LAYOUT_START_X + WINDOW_MARGIN + 2 * (COLUMN_X + WINDOW_MARGIN),
      LAYOUT_START_Y - 15, B_FALSE, COLUMN_X, 12, 1, "Customize parameters", 0,
      main_win, xpWidgetClass_Caption);
  (void)create_widget_rel(LAYOUT_START_X + 2 * (COLUMN_X + WINDOW_MARGIN),
                          LAYOUT_START_Y, B_FALSE, COLUMN_X, COLUMN_Y, 1, "", 0,
                          main_win, xpWidgetClass_SubWindow);

#define LAYOUT_BUTTON(var, text, type, behavior, tooltip)                      \
  do {                                                                         \
    buttons.var = create_widget_rel(x, y + 2, B_FALSE, 20, BUTTON_HEIGHT - 5,  \
                                    1, "", 0, main_win, xpWidgetClass_Button); \
    XPSetWidgetProperty(buttons.var, xpProperty_ButtonType, xpRadioButton);    \
    XPSetWidgetProperty(buttons.var, xpProperty_ButtonBehavior,                \
                        xpButtonBehavior##behavior);                           \
    (void)create_widget_rel(x + 20, y, B_FALSE, BUTTON_WIDTH - 20,             \
                            BUTTON_HEIGHT - 5, 1, text, 0, main_win,           \
                            xpWidgetClass_Caption);                            \
  ...
  LAYOUT_BUTTON(enabled, "Enabled", PushButton, CheckBox, enabled_tooltip);
  LAYOUT_BUTTON(use_imperial, "Call out distances in feet", PushButton,
                CheckBox, use_imperial_tooltip);
  LAYOUT_BUTTON(us_runway_numbers, "US runway numbers", PushButton, CheckBox,
                us_runway_numbers_tooltip);
  LAYOUT_BUTTON(voice_female, "Voice gender female", PushButton, CheckBox,
                voice_female_tooltip);
  LAYOUT_BUTTON(speak_units, "Speak units", PushButton, CheckBox,
                speak_units_tooltip);
  LAYOUT_BUTTON(say_deep_landing, "Say 'DEEP LANDING'", PushButton, CheckBox,
                say_deep_landing_tooltip);
#ifndef XRAAS_IS_EMBEDDED
  LAYOUT_BUTTON(allow_helos, "Start up in helicopters", PushButton, CheckBox,
                allow_helos_tooltip);
  LAYOUT_BUTTON(startup_notify, "Show startup notification", PushButton,
                CheckBox, startup_notify_tooltip);
  LAYOUT_BUTTON(auto_disable_notify, "Notify when X-RAAS auto-inhibits",
                PushButton, CheckBox, auto_disable_notify_tooltip);
#endif /* !XRAAS_IS_EMBEDDED */
  LAYOUT_BUTTON(disable_ext_view, "Silence in external views", PushButton,
                CheckBox, disable_ext_view_tooltip);
  LAYOUT_BUTTON(nd_alerts_enabled, "Visual alerts", PushButton, CheckBox,
                nd_alerts_enabled_tooltip);
  LAYOUT_BUTTON(nd_alert_overlay_enabled, "Visual alert overlay", PushButton,
                CheckBox, nd_alert_overlay_enabled_tooltip);
  LAYOUT_BUTTON(nd_alert_overlay_force,
                "Always show visual alerts using overlay", PushButton, CheckBox,
                nd_alert_overlay_force_tooltip);
#ifndef XRAAS_IS_EMBEDDED
  LAYOUT_BUTTON(override_electrical, "Override electrical check", PushButton,
                CheckBox, override_electrical_tooltip);
#endif /* !XRAAS_IS_EMBEDDED */
  LAYOUT_BUTTON(override_replay, "Show in replay mode", PushButton, CheckBox,
                override_replay_tooltip);
  LAYOUT_BUTTON(use_tts, "Use Text-To-Speech", PushButton, CheckBox,
                use_tts_tooltip);
#if LIN
  XPSetWidgetProperty(buttons.use_tts, xpProperty_Enabled, 0);
#endif /* !LIN */
  LAYOUT_BUTTON(openal_shared, "Shared audio driver context", PushButton,
                CheckBox, openal_shared_tooltip);

  x = LAYOUT_START_X + COLUMN_X + WINDOW_MARGIN;
  y = LAYOUT_START_Y + WINDOW_MARGIN / 2;

  for (int i = 0; i < NUM_MONITORS; i++) {
    LAYOUT_BUTTON(monitors[i], monitor_names[i], PushButton, CheckBox,
                  monitor_tooltips[i]);
#if ACF_TYPE == FF_A320_ACF_TYPE
    if (i == ON_RWY_FLAP_MON) {
      XPSetWidgetProperty(buttons.monitors[i], xpProperty_Enabled, 0);
    }
#endif /* ACF_TYPE == FF_A320_ACF_TYPE */
  }
...
#undef LAYOUT_BUTTON
  ...
  y = LAYOUT_START_Y + WINDOW_MARGIN / 2;
  scrollbars.voice_volume = layout_scroll_control(
      main_win, tts, &main_win_scrollbar_cbs, x, y, "Audio volume", 0, 100, 10,
      B_FALSE, 1.0, "%", NULL, voice_volume_tooltip);
  y += TEXT_FIELD_HEIGHT;
#if ACF_TYPE == NO_ACF_TYPE
  text_fields.min_engines =
      layout_text_field(main_win, tts, x, y, "Minimum number of engines", 1,
                        NULL, min_engines_tooltip, B_FALSE);
  y += TEXT_FIELD_HEIGHT;
  text_fields.min_mtow = layout_text_field(main_win, tts, x, y, "Minimum MTOW",
                                           6, "kg", min_mtow_tooltip, B_FALSE);
  y += TEXT_FIELD_HEIGHT;
#endif /* ACF_TYPE == NO_ACF_TYPE */
  text_fields.min_takeoff_dist =
      layout_text_field(main_win, tts, x, y, "Minimum takeoff distance", 4, "m",
                        min_takeoff_dist_tooltip, B_FALSE);
  y += TEXT_FIELD_HEIGHT;
  text_fields.min_landing_dist =
      layout_text_field(main_win, tts, x, y, "Minimum landing distance", 4, "m",
                        min_landing_dist_tooltip, B_FALSE);
  y += TEXT_FIELD_HEIGHT;
  text_fields.min_rotation_dist =
      layout_text_field(main_win, tts, x, y, "Minimum rotation distance", 4,
                        "m", min_rotation_dist_tooltip, B_FALSE);
  y += TEXT_FIELD_HEIGHT;
  text_fields.min_rotation_angle =
      layout_text_field(main_win, tts, x, y, "Minimum rotation angle", 4, "deg",
                        min_rotation_angle_tooltip, B_FALSE);
  y += TEXT_FIELD_HEIGHT;
  text_fields.stop_dist_cutoff =
      layout_text_field(main_win, tts, x, y, "Runway remaining cutoff length",
                        4, "m", stop_dist_cutoff_tooltip, B_FALSE);
  y += TEXT_FIELD_HEIGHT;
  text_fields.on_rwy_warn_initial = layout_text_field(
      main_win, tts, x, y, "Runway extended holding (initial)", 3, "sec",
      on_rwy_warn_initial_tooltip, B_FALSE);
  y += TEXT_FIELD_HEIGHT;
  text_fields.on_rwy_warn_repeat =
      layout_text_field(main_win, tts, x, y, "Runway extended holding (repeat)",
                        3, "sec", on_rwy_warn_repeat_tooltip, B_FALSE);
  y += TEXT_FIELD_HEIGHT;
  text_fields.on_rwy_warn_max_n =
      layout_text_field(main_win, tts, x, y, "Runway extended holding maximum",
                        2, NULL, on_rwy_warn_max_n_tooltip, B_FALSE);
  y += TEXT_FIELD_HEIGHT;
  scrollbars.gpa_limit_mult = layout_scroll_control(
      main_win, tts, &main_win_scrollbar_cbs, x, y, "GPA limit multiplier", 0,
      100, 10, B_FALSE, 0.1, NULL, NULL, gpa_limit_mult_tooltip);
  y += TEXT_FIELD_HEIGHT;
  scrollbars.gpa_limit_max = layout_scroll_control(
      main_win, tts, &main_win_scrollbar_cbs, x, y, "GPA limit maximum", 0, 100,
      10, B_FALSE, 0.1, "deg", NULL, gpa_limit_max_tooltip);
  y += TEXT_FIELD_HEIGHT;
  text_fields.long_land_lim_abs =
      layout_text_field(main_win, tts, x, y, "Long landing absolute limit", 4,
                        "m", long_land_lim_abs_tooltip, B_FALSE);
  y += TEXT_FIELD_HEIGHT;
  scrollbars.long_land_lim_fract =
      layout_scroll_control(main_win, tts, &main_win_scrollbar_cbs, x, y,
                            "Long landing limit fraction", 0, 100, 10, B_FALSE,
                            0.01, NULL, NULL, long_land_lim_fract_tooltip);
  y += TEXT_FIELD_HEIGHT;
#if ACF_TYPE == NO_ACF_TYPE
  scrollbars.min_landing_flap = layout_scroll_control(
      main_win, tts, &main_win_scrollbar_cbs, x, y, "Minimum landing flaps", 0,
      100, 10, B_FALSE, 0.01, NULL, NULL, min_landing_flap_tooltip);
  y += TEXT_FIELD_HEIGHT;
  scrollbars.min_takeoff_flap = layout_scroll_control(
      main_win, tts, &main_win_scrollbar_cbs, x, y, "Minimum takeoff flaps", 0,
      100, 10, B_FALSE, 0.01, NULL, NULL, min_takeoff_flap_tooltip);
  y += TEXT_FIELD_HEIGHT;
  scrollbars.max_takeoff_flap = layout_scroll_control(
      main_win, tts, &main_win_scrollbar_cbs, x, y, "Maximum takeoff flaps", 0,
      100, 10, B_FALSE, 0.01, NULL, NULL, max_takeoff_flap_tooltip);
  y += TEXT_FIELD_HEIGHT;
#endif /* ACF_TYPE == NO_ACF_TYPE */
  text_fields.nd_alert_timeout =
      layout_text_field(main_win, tts, x, y, "Visual alert timeout", 3, "sec",
                        nd_alert_timeout_tooltip, B_FALSE);
  y += TEXT_FIELD_HEIGHT;
  scrollbars.nd_alert_filter = layout_scroll_control(
      main_win, tts, &main_win_scrollbar_cbs, x, y, "Visual alert filter", 0, 2,
      1, B_FALSE, 1, NULL, nd_alert2str, nd_alert_filter_tooltip);
  y += TEXT_FIELD_HEIGHT;
  text_fields.nd_alert_overlay_font =
      layout_text_field(main_win, tts, x, y, "Visual alert overlay font", 500,
                        NULL, nd_alert_overlay_font_tooltip, B_TRUE);
  y += TEXT_FIELD_HEIGHT;
  text_fields.nd_alert_overlay_font_size =
      layout_text_field(main_win, tts, x, y, "Visual alert overlay font size",
                        3, NULL, nd_alert_overlay_font_size_tooltip, B_FALSE);
  y += TEXT_FIELD_HEIGHT;
...
#define LAYOUT_PUSH_BUTTON(var, x, y, w, h, label, tooltip)                    \
  ...
  LAYOUT_PUSH_BUTTON(save_liv_conf, WINDOW_MARGIN, MAIN_WINDOW_HEIGHT - 70,
                     BUTTON_WIDTH, 18, "SAVE airline configuration",
                     save_liv_tooltip);
  LAYOUT_PUSH_BUTTON(reset_liv_conf,
                     MAIN_WINDOW_WIDTH - BUTTON_WIDTH - WINDOW_MARGIN,
                     MAIN_WINDOW_HEIGHT - 70, BUTTON_WIDTH, 18,
                     "RESET airline configuration", reset_liv_tooltip);

#ifdef XRAAS_IS_EMBEDDED
  LAYOUT_PUSH_BUTTON(save_acf_conf, WINDOW_MARGIN, MAIN_WINDOW_HEIGHT - 50,
                     BUTTON_WIDTH, 18, "SAVE configuration", save_acf_tooltip);
  LAYOUT_PUSH_BUTTON(reset_acf_conf,
                     MAIN_WINDOW_WIDTH - BUTTON_WIDTH - WINDOW_MARGIN,
                     MAIN_WINDOW_HEIGHT - 50, BUTTON_WIDTH, 18,
                     "RESET configuration", reset_acf_tooltip);
#else  /* !XRAAS_IS_EMBEDDED */
  LAYOUT_PUSH_BUTTON(save_acf_conf, WINDOW_MARGIN, MAIN_WINDOW_HEIGHT - 50,
                     BUTTON_WIDTH, 18, "SAVE aircraft configuration",
                     save_acf_tooltip);
  LAYOUT_PUSH_BUTTON(save_glob_conf, WINDOW_MARGIN, MAIN_WINDOW_HEIGHT - 30,
                     BUTTON_WIDTH, 18, "SAVE global configuration",
                     save_glob_tooltip);
  ...
  LAYOUT_PUSH_BUTTON(reset_glob_conf,
                     MAIN_WINDOW_WIDTH - BUTTON_WIDTH - WINDOW_MARGIN,
                     MAIN_WINDOW_HEIGHT - 30, BUTTON_WIDTH, 18,
...
#undef LAYOUT_PUSH_BUTTON
  ...
  create_widget_rel(
      2 * WINDOW_MARGIN + BUTTON_WIDTH, MAIN_WINDOW_HEIGHT - 75, B_FALSE,
      MAIN_WINDOW_WIDTH - 2 * BUTTON_WIDTH - 4 * WINDOW_MARGIN,
      TEXT_FIELD_HEIGHT, 1, COPYRIGHT1, 0, main_win, xpWidgetClass_Caption);
  create_widget_rel(
      2 * WINDOW_MARGIN + BUTTON_WIDTH, MAIN_WINDOW_HEIGHT - 62, B_FALSE,
      MAIN_WINDOW_WIDTH - 2 * BUTTON_WIDTH - 4 * WINDOW_MARGIN,
      TEXT_FIELD_HEIGHT, 1, COPYRIGHT2, 0, main_win, xpWidgetClass_Caption);
  create_widget_rel(
      2 * WINDOW_MARGIN + BUTTON_WIDTH, MAIN_WINDOW_HEIGHT - 49, B_FALSE,
      MAIN_WINDOW_WIDTH - 2 * BUTTON_WIDTH - 4 * WINDOW_MARGIN,
      TEXT_FIELD_HEIGHT, 1, TOOLTIP_HINT, 0, main_win, xpWidgetClass_Caption);

  text_fields.status_msg = create_widget_rel(
      2 * WINDOW_MARGIN + BUTTON_WIDTH, MAIN_WINDOW_HEIGHT - 27, B_FALSE,
      MAIN_WINDOW_WIDTH - 2 * BUTTON_WIDTH - 4 * WINDOW_MARGIN, 18, 1, "", 0,
      main_win, xpWidgetClass_Caption);
}
...
#define UPDATE_BUTTON_STATE(button)                                            \
  ...
  for (int i = 0; i < NUM_MONITORS; i++)
    XPSetWidgetProperty(buttons.monitors[i], xpProperty_ButtonState,
                        xraas_state->config.monitors[i]);
...
#define UPDATE_TEXT_FIELD(field, fmt)                                          \
  ...
  if (strcmp(xraas_state->config.nd_alert_overlay_font,
    ...
    XPSetWidgetDescriptor(text_fields.nd_alert_overlay_font, "default");
  else
...
static int command_cb(XPLMCommandRef cmd, XPLMCommandPhase phase,
  ...
  if (phase == xplm_CommandBegin) {
    if (cmd == toggle_cfg_gui_cmd) {
      setup_window->SetVisible(!setup_window->GetVisible());
      ///////////////////// TODO remove me
...
void gui_init(void) {
  if (gui_inited)
    return;
  tooltip_init(); /// TODO remove it
  create_menu();
  create_main_window(); //// TODO remove it
  create_setup_window();
  register_commands();
  gui_inited = B_TRUE;
}
...
void gui_update(void) {
  ...
  XPLMCheckMenuItem(root_menu, dbg_gui_menu_item,
                    xraas_state->config.debug_graphical ? xplm_Menu_Checked
                                                        : xplm_Menu_Unchecked);
  update_main_window(); // TODO remove it
}
