...
find_nearest_curarpt(void)
{
	...
	vect3_t pos_ecef = geo2ecef_ft(GEO_POS3(adc->lat, adc->lon, adc->elev),
	    &wgs84);
	...
	if (cur_arpt != NULL) {
		...
		*TL = cur_arpt->TL;
		...
		if (arpt_ref != 0) {
			...
			arpt_ecef = geo2ecef_ft(GEO_POS3(outLat, outLon, 0),
			    &wgs84);
			pos_ecef = geo2ecef_ft(GEO_POS3(adc->lat, adc->lon, 0),
			    &wgs84);
		}
